...
const CommunityDetail = () => {
  ...
  const truncateTitle = (title, maxLength = 30) => {
    ...
    return title.length > maxLength
      ? `${title.slice(0, maxLength)}···.${title.split('.').pop().toLowerCase()}`
      : title;
  };
  ...
  const handleAddReply = async (parentId) => {
    ...
    try {
      await axios.post(
        `${API_BASE_URL}/community/${posts.communityId}/comments`,
        {
      ...
      <div key={topComment.communityComentId} className='comment-item'>
        <span className='writerAndOption'>
          ...
          {(String(userId) === String(posts.employeeId) ||
            topComment.writerName === userName) && (
        ...
        {editCommentId === topComment.communityComentId ? (
          <div className='edit-input'>
            ...
            />
            <button
              onClick={() => handleEditComment(topComment.communityComentId)}
            >
              저장
            </button>
          ...
          <>
            ...
            <p className='commentDate'>
              {topComment.createdAt?.substring(0, 16).replace('T', ' ')}
            </p>
        ...
        {replyTargetId === topComment.communityComentId && (
          <div className='reply-input'>
            ...
            />
            <button
              onClick={() => handleAddReply(topComment.communityComentId)}
            >
              등록
            </button>
              ...
              <div key={idx}>
                ...
                >
                  <img
                    src={
                      fileIconMap[
                        attachments[0].split('.').pop().toLowerCase()
                      ] || '/icons/default.png'
                    }
                    alt={attachments[0].split('.').pop().toLowerCase()}
